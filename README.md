# Bedrock Experience

![Java 8](https://img.shields.io/badge/language-Java%208-9B599A.svg?style=flat-square)
[![GitHub license](https://img.shields.io/github/license/LambdAurora/BedrockExperience?style=flat-square)](https://raw.githubusercontent.com/LambdAurora/BedrockExperience/master/LICENSE)
![Environment: Client](https://img.shields.io/badge/environment-client-1976d2?style=flat-square)
![Mod loader: Fabric](https://img.shields.io/badge/modloader-Fabric-1976d2?style=flat-square&logo=data:image/png;base64,)
![Version](https://img.shields.io/github/v/tag/LambdAurora/BedrockExperience?label=version&style=flat-square)

A Fabric Minecraft mod which brings some features from Bedrock Edition to Java.

## What's this mod?

Do you like some features of Bedrock Edition, like the minimalist coordinates indicator?
Do you want those features to be on Java Edition? Well, good news! This mod brings some of these features!

This mod can be seen as a port of [Console Experience](https://github.com/Fuzss/consoleexperience) to Fabric.

## ✅ Features:

 - Coordinates indicator

## Build

Just do `./gradlew build` and everything should build just fine!
